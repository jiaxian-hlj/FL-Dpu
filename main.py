...
def main(args):
    ...
    for i in folds:
        ...
        if len(train_datasets)>1:
            ...
            results, test_auc, val_auc, test_error, val_error,test_bacc,test_F1  = train_fl(datasets, i, args)
        else:
            train_dataset = train_datasets[0] 
            print('training: {}, validation: {}, testing: {}'.format(len(train_dataset), len(val_dataset), len(test_dataset)))
            datasets = (train_dataset, val_dataset, test_dataset)
            results, test_auc, val_auc, test_error, val_error,test_bacc,test_F1  = train(datasets, i, args)
    ...
    final_df = pd.DataFrame({'folds': folds, 'test_auc': all_test_auc, 
        'val_auc': all_val_auc, 'test_error': all_test_error, 'val_error' : all_val_error,'test_bacc': all_test_bacc, 'test_F1' : all_test_F1})
...
parser.add_argument('--opt', type=str, choices = ['adam', 'sgd','rmsprop'], default='rmsprop')
